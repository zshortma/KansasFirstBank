<template>
    <div ref="container" :class="cx('root')" :style="sx('root')" @click="onContainerClick" :data-p="containerDataP" v-bind="ptmi('root')">
        ...
        <slot v-if="isClearIconVisible" name="clearicon" :class="cx('clearIcon')" :clearCallback="onClearClick">
            <Times :class="[cx('clearIcon')]" @click="onClearClick" v-bind="ptm('clearIcon')" />
        </slot>
        <ul
            v-if="multiple"
            ref="multiContainer"
        ...
        >
            ...
            >
                <slot name="chip" :class="cx('pcChip')" :value="option" :index="i" :removeCallback="(event) => removeOption(event, i)" v-bind="ptm('pcChip')">
                    <Chip :class="cx('pcChip')" :label="getOptionLabel(option)" :removeIcon="chipIcon" removable :unstyled="unstyled" @remove="removeOption($event, i)" :data-p-focused="focusedMultipleOptionIndex === i" :pt="ptm('pcChip')">
                        <template #removeicon>
                            <slot name="chipicon" :class="cx('chipIcon')" :index="i" :removeCallback="(event) => removeOption(event, i)" />
                        </template>
                    </Chip>
                </slot>
            </li>
        ...
        <slot v-if="searching || loading" :class="cx('loader')" name="loader">
            <i v-if="loader" :class="['pi-spin', cx('loader'), loader]" aria-hidden="true" :data-p-has-dropdown="dropdown" v-bind="ptm('loader')" />
            <Spinner v-else-if="loading" :class="cx('loader')" spin aria-hidden="true" :data-p-has-dropdown="dropdown" v-bind="ptm('loader')" />
        </slot>
        <slot name="dropdown" :toggleCallback="(event) => onDropdownClick(event)">
            <button
                v-if="dropdown"
                ref="dropdownButton"
                type="button"
            ...
            >
                <slot name="dropdownicon" :class="dropdownIcon">
                    <component :is="dropdownIcon ? 'span' : 'ChevronDown'" :class="dropdownIcon" v-bind="ptm('dropdownIcon')" />
                </slot>
            </button>
        </slot>
...
export default {
    ...
    methods: {
        ...
        alignOverlay() {
            ...
            const own = this.$refs.container;
            const parent = own?.parentElement;
            const target = parent?.dataset?.pcName === 'inputtags' ? parent : own;
            ...
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    if (this.searching) {
                        ...
                        if (this.showEmptyMessage || this.visibleOptions.length > 0) {
                            this.show();
                            this.focusedOptionIndex = this.overlayVisible && this.autoOptionFocus ? this.findFirstFocusedOptionIndex() : -1;
                        }
                    }
