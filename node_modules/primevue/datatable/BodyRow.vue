<template>
    <template v-if="!empty">
        <tr v-if="templates['groupheader'] && rowGroupMode === 'subheader' && shouldRenderRowGroupHeader" :class="cx('rowGroupHeader')" :style="rowGroupHeaderStyle" role="row" v-bind="ptm('rowGroupHeader')">
            <td :colspan="rowGroupHeaderColspan" v-bind="{ ...getColumnPT('bodycell'), ...ptm('rowGroupHeaderCell') }">
                <button v-if="expandableRowGroups" :class="cx('rowToggleButton')" @click="onRowGroupToggle" type="button" v-bind="ptm('rowToggleButton')">
                    ...
                    <template v-else>
                        ...
                        <ChevronDown v-else-if="isRowGroupExpanded && !expandedRowIcon" :class="cx('rowToggleIcon')" v-bind="ptm('rowToggleIcon')" />
                        <span v-else-if="!isRowGroupExpanded && collapsedRowIcon" :class="[cx('rowToggleIcon'), collapsedRowIcon]" v-bind="ptm('rowToggleIcon')" />
                        <ChevronRight v-else-if="!isRowGroupExpanded && !collapsedRowIcon" :class="cx('rowToggleIcon')" v-bind="ptm('rowToggleIcon')" />
                    </template>
                </button>
        ...
        <tr
            ...
            role="row"
            :aria-selected="selectionMode ? selected : null"
            @click="onRowClick"
            ...
            :data-p-selected-contextmenu="contextMenuSelected"
        >
        ...
        <tr v-if="templates['groupfooter'] && rowGroupMode === 'subheader' && shouldRenderRowGroupFooter" :class="cx('rowGroupFooter')" role="row" v-bind="ptm('rowGroupFooter')">
            <td :colspan="rowGroupHeaderColspan" v-bind="{ ...getColumnPT('bodycell'), ...ptm('rowGroupFooterCell') }">
...
import { isNotEmpty, equals as objectEquals, resolveFieldData } from '@primeuix/utils/object';
...
export default {
    ...
    methods: {
        ...
        equals(data1, data2) {
            return this.compareSelectionBy === 'equals' ? data1 === data2 : objectEquals(data1, data2, this.dataKey);
        },
        ...
        rowGroupHeaderColspan() {
            if (this.columns) {
                ...
                this.columns.forEach((column) => {
                    ...
                    if (!this.columnProp(column, 'hidden') && this.groupRowsBy !== this.columnProp(column, 'field')) colspan++;
                });
