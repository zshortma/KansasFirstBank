            ...
            <slot name="incrementbutton" :listeners="upButtonListeners">
                <button :class="[cx('incrementButton'), incrementButtonClass]" v-on="upButtonListeners" :disabled="disabled" :tabindex="-1" aria-hidden="true" type="button" v-bind="ptm('incrementButton')" :data-p="dataP">
                    <slot name="incrementicon">
                        <component :is="incrementIcon ? 'span' : 'AngleUp'" :class="incrementIcon" v-bind="ptm('incrementIcon')" data-pc-section="incrementicon" />
                    </slot>
                </button>
            </slot>
            <slot name="decrementbutton" :listeners="downButtonListeners">
                <button :class="[cx('decrementButton'), decrementButtonClass]" v-on="downButtonListeners" :disabled="disabled" :tabindex="-1" aria-hidden="true" type="button" v-bind="ptm('decrementButton')" :data-p="dataP">
                    <slot name="decrementicon">
                        <component :is="decrementIcon ? 'span' : 'AngleDown'" :class="decrementIcon" v-bind="ptm('decrementIcon')" data-pc-section="decrementicon" />
                    </slot>
                </button>
            </slot>
        </span>
...
export default {
    ...
    methods: {
        ...
        formatValue(value) {
            if (value != null) {
                ...
                if (this.format) {
                    ...
                    if (this.prefix) {
                        if (value < 0) {
                            const minusSign = new Intl.NumberFormat(this.locale, { useGrouping: false }).format(-1).trim().replace(this._numeral, '') || '-';
        ...
        onInputCompositionEnd(event) {
            ...
            event.target.value = this.compositionValue ?? '';
            event.target.setSelectionRange(this.compositionStart, this.compositionEnd);

            for (const char of data) {
                if (this.isNumeralChar(char)) {
                    this.insert(event, char, { isDecimalSign: this.isDecimalSign(char), isMinusSign: this.isMinusSign(char) });
                }
            }
        },
        ...
        insert(event, text, sign = { isDecimalSign: false, isMinusSign: false }) {
            ...
            if (sign.isMinusSign) {
                ...
                const prefixBoundary = (this.prefixChar || '').length;
                const isAtPrefixBoundary = prefixBoundary > 0 && selectionStart === prefixBoundary;
